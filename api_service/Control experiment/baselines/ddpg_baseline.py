...
def run_ddpg_baseline(iterations: int = 20, seed: int = None, collect_latency_details: bool = True) -> Dict[str, float]:
    """DDPG基线 - 单次运行
    ...
    if seed is not None:
        ...
        logging.info(f"[Baseline-DDPG] 使用随机种子: {seed}")
    
    logging.info("[Baseline-DDPG] 初始化...")
    ...
    model = DDPG(
        ...
        batch_size=64, gamma=0.95, action_noise=action_noise,
        verbose=0, device='cuda' if torch.cuda.is_available() else 'cpu'
    )
    
    logging.info("[Baseline-DDPG] 训练...")
    # 总步数与BO baseline的迭代次数一致，保证公平比较
    total_steps = iterations  # 20步，与BO的20次迭代对应
    logging.info(f"[Baseline-DDPG] 总训练步数: {total_steps} (与BO baseline的{iterations}次迭代对应)")
    model.learn(total_timesteps=total_steps, log_interval=5)
    
    logging.info("[Baseline-DDPG] 评估...")
    obs, _ = env.reset()
    ...
    if collect_latency_details:
        ...
        logging.info(f"[Baseline-DDPG] 收集了 {len(env.all_latencies)} 个延迟数据点")
    
    logging.info(f"[Baseline-DDPG] 完成: {best_metrics}")
    return best_metrics


def run_ddpg_multiple_trials(num_trials: int = 10, iterations: int = 20, 
    ...
    logging.info("=" * 80)
    logging.info(f"[DDPG实验] 开始运行 {num_trials} 次独立实验")
    logging.info(f"[DDPG实验] 每次实验训练步数: {iterations}")
    logging.info(f"[DDPG实验] 基础随机种子: {base_seed}")
    logging.info("=" * 80)
    ...
    for trial in range(num_trials):
        ...
        logging.info("")
        logging.info(f"{'='*80}")
        logging.info(f"[DDPG实验] Trial {trial + 1}/{num_trials} (seed={seed})")
        logging.info(f"{'='*80}")
        
        try:
            result = run_ddpg_baseline(iterations=iterations, seed=seed, collect_latency_details=True)  # 🔥 收集延迟详情
            ...
            if 'latency_distribution' in result and len(result['latency_distribution']) > 0:
                ...
                logging.info(f"[DDPG实验] Trial {trial + 1} 完成: QPS={result['qps']:.2f}, "
            ...
            logging.error(f"[DDPG实验] Trial {trial + 1} 失败: {e}")
            import traceback
            traceback.print_exc()
    ...
    if all_results:
        ...
        if len(all_latencies_combined) > 0:
            ...
            logging.info("")
            logging.info("=" * 80)
            logging.info("[DDPG实验] 统计摘要 (n={})".format(len(all_results)))
            logging.info("=" * 80)
    ...
    if save_path is None:
        save_path = Path(__file__).parent.parent / "result" / f"ddpg_baseline_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    else:
    ...
    logging.info(f"[DDPG实验] 结果已保存到: {save_path}")
...
if __name__ == "__main__":
    # 运行10次实验
    results = run_ddpg_multiple_trials(num_trials=10, iterations=20, base_seed=42)
