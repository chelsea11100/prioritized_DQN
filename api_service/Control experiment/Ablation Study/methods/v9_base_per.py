...
# -*- coding: utf-8 -*-
"""
V9 Base (PER only): V9基础版本，仅保留优先级经验回放
移除: Meta-Learning + Bayesian Optimization（保留NAS架构以兼容预训练模型）
"""
import logging
import sys
import os
...
def run_v9_base_per(model_path: str = None) -> Dict[str, float]:
    """运行V9基础版本（仅保留Prioritized DQN）
    ...
    logging.info("[Ablation-V9-Base-PER] 初始化V9基础版本（仅保留PER）...")
    logging.info(f"[Ablation-V9-Base-PER] 使用模型: {model_path}")
    ...
    # 创建Agent（禁用所有增强功能，只保留PER）
    logging.info("🧩 创建V9 Agent（仅启用PER，禁用Meta和Bayesian）...")
    ...
    agent = EnhancedDQNAgentV9(
        ...
        enable_bayesian_opt=False,    # ⬅️ 关闭贝叶斯优化
        use_nas_architecture=True     # 保留NAS架构（为了兼容预训练模型）
    )
    ...
    try:
        ...
        if ablation_flags.get('enable_bayesian_opt', True):
            raise ValueError("❌ 消融实验失败：贝叶斯优化未被正确禁用！")
        
        training_stats = agent.get_training_stats()
        logging.info(f"📊 预训练轮数: {training_stats.get('episode_count', 0)}")
        logging.info("⚠️ 此版本仅使用Prioritized DQN，所有增强功能已禁用")
